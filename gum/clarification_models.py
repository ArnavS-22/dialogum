...
class ClarifyingQuestion(Base):
    """Stores generated clarifying questions for propositions.
    
    Each proposition-factor combination that was flagged for clarification
    can have one or more generated clarifying questions. These questions
    are generated by the question_engine to help users understand what
    aspects of the proposition might need clarification.
    
    Attributes:
        id (int): Primary key
        proposition_id (int): Foreign key to propositions table
        analysis_id (int): Foreign key to clarification_analyses table
        factor_name (str): Name of the triggering factor (e.g., "inferred_intent")
        factor_id (int): ID of the factor (1-12)
        
        question (str): The generated clarifying question text
        reasoning (str): Explanation of why this question is needed
        evidence (list): Observation IDs that provided evidence for this question
        
        generation_method (str): Method used ("few-shot" or "controlled_qg")
        model_used (str): LLM model used to generate the question
        validation_passed (bool): Whether validation checks passed
        validation_warnings (list): List of validation warning messages
    ...
    analysis_id: Mapped[Optional[int]] = mapped_column(
        ForeignKey("clarification_analyses.id", ondelete="CASCADE"),
        nullable=True,
        index=True
    )
    
    # Factor information
    factor_name: Mapped[str] = mapped_column(String(50), nullable=False, index=True)
    factor_id: Mapped[int] = mapped_column(Integer, nullable=False)
    factor_score: Mapped[float] = mapped_column(Float, nullable=False)  # Score for this specific factor
    ...
    proposition: Mapped[Proposition] = relationship(
        "Proposition",
        backref="clarifying_questions"
    )
    analysis: Mapped[Optional["ClarificationAnalysis"]] = relationship(
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"<ClarifyingQuestion(id={self.id}, prop_id={self.proposition_id}, "
            f"factor={self.factor_name}, question='{preview}')>"
        )
